...
def execute_query(query, params=None):
    myconnection = pymysql.connect(host='localhost', user='root', password='root', database='swathi')
    try:
...
def generate_query(bustype, route, price_min, price_max, rating_min, rating_max, availability):
    query = "SELECT * FROM swathi.cleaned_redbus WHERE 1=1"
...
def main():
    ...
    bustypes_df = execute_query("SELECT DISTINCT Bus_Type FROM swathi.cleaned_redbus")
    routes_df = execute_query("SELECT DISTINCT Bus_Route FROM swathi.cleaned_redbus")
